...
// console.log(new Date().toString(), 'switch to 1')

    const targets = await targetHandler.getCriTargets(arg);
// console.log(new Date().toString(), 'switch to 2')
...
// console.log(new Date().toString(), 'switch to 3')
  await targetHandler.switchBrowserContext(targetId);
// console.log(new Date().toString(), 'switch to 4')
  await connect_to_cri(targetId);
  descEvent.emit('success', message);
// console.log(new Date().toString(), 'switch to 5')
};
...
module.exports.openTab = async (targetUrl, options = {}) => {
  ...
  // console.log('xxx doActionAwaitingNavigation openTab', targetUrl);
...
module.exports.openIncognitoWindow = async (url, options = {}) => {
  ...
  if (url !== 'about:blank') {
    // console.log('xxx doActionAwaitingNavigation openIncognitoWindow', url);
...
module.exports.closeIncognitoWindow = async (arg) => {
  ...
  console.log(new Date(), 'xxx closeIncognitoWindow start');
  ...
  console.log(new Date(), 'xxx closeIncognitoWindow, hasClosedActiveBrowserContext = ', hasClosedActiveBrowserContext);

  if (hasClosedActiveBrowserContext) {
    console.log(new Date(), 'xxx closeIncognitoWindow, waiting for reconnect');

    const promiseReconnect = new Promise((resolve) => {
      eventHandler.once('reconnected', resolve);
    });
    await promiseReconnect;
    console.log(new Date(), 'xxx closeIncognitoWindow reconnect finished', hasClosedActiveBrowserContext);
  }

  console.log(new Date(), 'xxx closeIncognitoWindow, finished');
...
module.exports.goto = async (
  ...
  options = setNavigationOptions(options);
  let response;
  // console.log('xxx doActionAwaitingNavigation goto', url);
  await doActionAwaitingNavigation(options, async () => {
...
module.exports.reload = async (
  ...
  validate();
  options = setNavigationOptions(options);
  // console.log('xxx doActionAwaitingNavigation reload');
  await doActionAwaitingNavigation(options, async () => {
...
const _go = async (delta, options) => {
  ...
  options = setNavigationOptions(options);
  // console.log('xxx doActionAwaitingNavigation goForward');  
  await doActionAwaitingNavigation(options, async () => {
...
async function _press(keys, options) {
  // console.log('xxx doActionAwaitingNavigation press', keys);  
  await doActionAwaitingNavigation(options, async () => {
...
module.exports.scrollTo = async (selector, options = {}) => {
  ...
  // console.log('xxx doActionAwaitingNavigation scrollTo', selector.toLogString());  
  await doActionAwaitingNavigation(options, async () => {
...
module.exports.evaluate = async (selector, callback, options = {}) => {
  ...
  options = setNavigationOptions(options);
  // console.log('xxx doActionAwaitingNavigation evaluate', selector.toLogString());    
  await doActionAwaitingNavigation(options, async () => {
